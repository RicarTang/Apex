...
@celery.task(
    bind=True,
    name="src.core.celery.task.scheduled_task.statistics_dashbord_data",
    base=BaseTask,
)
def statistics_dashbord_data(self):
    """首页面板统计任务"""
    # 查询所有用例数量与定时任务数量
    sql_text = text(
        """
...
"""
    ...
    # 保存json格式至redis
    try:
        RedisService().redis_pool.set(
            "task:dashbord-statistics-data", json.dumps(dict(result))
        )
    except Exception:
        raise RedisError