...
class BaseTask(Task):
    ...
    def after_return(self, status, retval, einfo, task_id, *args, **kwargs):
        """任务返回后调用的处理程序"""
        log.info(
    ...
    def on_retry(self, exc, task_id, einfo, *args, **kwargs):
        """当要重试任务时，它由工作线程运行"""
        log.info(f"task:{task_id}准备重试,,{args},{kwargs},exc:{exc},einfo:{einfo}")

    def on_success(self, retval, task_id, *args, **kwargs):
        """任务执行成功回调"""
        log.info(f"task:{task_id}执行成功，task_name:{self.name}")
...
class BaseTaskWithTest(BaseTask):
    ...
    def on_success(self, retval, task_id, *args, **kwargs):
        ...
        log.info(
            f"task:{task_id}执行成功,pytest退出码:{exit_code},开始修改suite_id:{suite_id}的状态"
        )
