...
@router.get(
    "/me",
    summary="获取我的评论",
    response_model=ResultResponse[List[comment_schema.CommentTo]],
...
async def get_comments_me(current_user: Users = Depends(get_current_user)):
    ...
    return ResultResponse[List[comment_schema.CommentTo]](result=comments)
...
async def get_user_comment(
    ...
    return ResultResponse[List[comment_schema.CommentTo]](result=coms)
