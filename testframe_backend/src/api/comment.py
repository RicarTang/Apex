...
from ..schemas import ResultResponse, comment
...
@router.post(
    "/create", summary="发表评论", response_model=ResultResponse[comment.CommentTo]
)
async def create_comment(
    ...
    return ResultResponse[comment.CommentTo](result=com)


@router.get(
    "/me",
    summary="获取我的评论",
    response_model=ResultResponse[List[comment.CommentTo]],
    # response_model=ResultResponse[comment.CommentsTo],
)
async def get_comments_me(current_user: Users = Depends(get_current_user)):
    ...
    return ResultResponse[List[comment.CommentTo]](result=comments)
...
async def get_user_comment(
    ...
    return ResultResponse[List[comment.CommentTo]](result=coms)
